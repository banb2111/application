...
class Users extends  CI_Controller{
    ...
    public function index(){
        ...
        if(!$_SESSION['user_id']){
			...
			if($department_no==24||$department_no==29||$department_no==12||$department_no==-1){
				$data['huixiao']=1;
			}
    ...
    /**
     * [ajax_check_user 检测登录账号]
     * @author   zzr QQ:836663500
     * @datetime 2016-12-24T11:48:36+0800
    ...
    public function ajax_check_user(){
        ...
        if(trim($user[0]->password) == md5(trim($password))) {
            ...
            echo json_encode(array('s'=>'err','msg'=>'用户名错误'));
        }else{
            echo json_encode(array('s'=>'err','msg'=>'密码有误'));
        }
        exit();
    }
    ...
    public function  user_admin(){
		...
		if(count($_GET)>0){
			...
			$str.=" and e.department_no=".$department_id;
			}			
			if(strlen($status)>0){
			$str.=" and u.status=".$status;
			}
/* 			$result=$this->db	
			->select("*,u.id as uid,e.name,d.name as dname,c.name as cname,u.type as user_type")
			->from("user u")
				...
				->result();	 */
			//echo 
			$sql="select *,u.id as uid,e.name,d.name as dname,c.name as cname,u.type as user_type
			from nb_user as u 
			left join nb_employee as e on u.id=e.user_id 
			left join nb_department as d on d.id=e.department_no 
			left join nb_company as c on c.id=e.company_id 
			".$str." ";
			//die();
			$result=$this->db->query($sql)->result_array();
			//->result_array();
			//$result=$this->db->query("select * from nb_user")->result_array();
			//var_dump($result);die();
    ...
    public function department_forbidden(){
		...
		//echo 
		$sql2="select count(nb_employee.user_id) from nb_user left join nb_employee on nb_user.id=nb_employee.user_id 
		where nb_user.status=1 and nb_employee.department_no=".$_POST["nonow"];
		//DIE();
		$query2 = $this->db->query($sql2);
		foreach ($query2->result_array() as $row)
{
		$p1=$row ["count(nb_employee.user_id)"];
}
		foreach ($query1->result_array() as $row)
{
		...
		if($p1==0&&$p2==0){
		$sql="update nb_department set display=0 where id=".$_POST["nonow"];
    ...
    public function department_edit(){
		$sql="update nb_department set no=".$_POST["no"]." where id=".$_POST["nonow"];
		$query = $this->db->query($sql);
		redirect("users");
...
//        array_push($cus_status,$temp);
    ...
    public function get_users(){
        $depart_id = trim($this->input->post('department_id'));
        //指定销售的所有人
        $pagenum=$_POST['per_page']?$_POST['per_page']:1;
        // 按姓名搜索
        $serusername = trim($this->input->post('serchusername'));
        // 按在职离职状态
        $onwork = trim($this->input->post('onwork'));
        if($pagenum==1){
            $offset=0;
        }else{
            $offset=10 * ($pagenum-1);
        }
        if(!empty($serusername)){
            $this->db->like('e.name',$serusername);
        }elseif($depart_id!=null){
            // @zzr edit at 2016-12-23 18:21
            if(strpos($depart_id , ',')){
                $this->db->where('d.id in ('.$depart_id.')');
            }else{
                $this->db->where("d.id",$depart_id);    
            }
        }
        if(!empty($onwork) && $onwork == 'no'){
