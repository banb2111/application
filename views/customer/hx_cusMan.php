                            ...
                            <div class="mb10 row" id="exampleTableEventsToolbar" role="group" >
                                <div class="col-sm-4 col-md-3 col-lg-2">
                                    <select name="" id="sousuo_type" class="form-control">
                                        ...
                                        <option <?php if ($type==11){?> selected="selected"<?php }?> value="11">联系人</option>

                                        <option <?php if ($type==12){?> selected="selected"<?php }?> value="12" data-toggle="modal" data-target="#selbyuser">客户所有人</option>
                                ...
                                <div class="col-sm-4 col-md-3 col-lg-2">
                                    <select name="" id="sousuo_channel" class="form-control">
                                        <?php
                                          foreach($all_channels as $all_ch_val){
                                        ?>
                                        <option value="<?=$all_ch_val['id']?>" <?php if($type==$all_ch_val['id']) echo 'selected="selected"';?> ><?=$all_ch_val['channel_name']?></option>
                                        <?php } ?>
                                    </select>
                                </div>
                                <?php } ?>
                                    ...
                                    <?php foreach($customer as $k=>$v){?>
                                        ...
                                        <tr class="customer_tr_<?=$v->cus_id?>">
                                           ...
                                           <!--<?php if($v->will_status==1){ ?>
											...
											<td class="not" ><?php echo $v->ename; ?></td>
                                        </tr>
                                    <?php }?>
                                    <?php }else{ ?>
...
<!-- 按客户来源渠道搜索 -->
<div class="modal inmodal fade" id="selbyuser" tabindex="-1" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                <h4 class="modal-title" style="font-size:18px;font-weight: normal;">按客户所有人搜索</h4>
            </div>
            <div class="modal-body p10 clearfix">
                <form class="form-inline">
                  <div class="form-group">
                  ...
                  <button type='button' class="btn btn-primary" id="query_selbyuser">查询</button>
                </form>
                <table class="table mt10" id="selbyuser_table2">
                </table>
                 <span class="mes_pR pull-left">
                     <a onclick="up('#selbyuser')">上一页</a>
                     <a onclick="down('#selbyuser')">下一页</a>
                 </span>
            </div>
            <div class="modal-footer">
                <button class="btn btn-primary" id="sub_selbyuser" >确认</button>
                <button class="btn btn-default" id=""  data-dismiss="modal">取消</button>
            </div>
        </div>
    </div>
...
//        var arr = [];
    ...
    /**
     * [followstage_change 跟进阶段更新]
     * @author   zzr QQ:836663500
     * @datetime 2016-12-23T08:41:48+0800
     * @param    {[type]}                 obj [description]
     * @return   {[type]}                     [description]
     */
    function followstage_change(obj){
        var id=$("#custo_id").val();
        var status=obj.value;
        $.ajax({
            type: "POST",
            url: "<?=base_url();  ?>index.php/customer/update_followstage?",
            data: "id="+id+"&status1="+status,
            success: function(data){
                if(data){
                    cus_status(id);
...
<script>
    ...
    $(function(){
        ...
        $('.will').click(function(event) {
            var will_status;
            var will=$(this).html();
			//showInfo("重点客户数量不能超过"+will+"个",'error');
            if(num!=0||will=="是"){
    ...
    $("#sub_selbyuser").click(function(){
        ...
        //指定销售查询部门
        $('#query_selbyuser').click(function() {
            var department_id = $('#selbyuser').find('select').val();
